...
?>
    ...
    <?php
        ...
        $sql_search = "SELECT product_type.* FROM product_type"; 
        $search = $_POST['search'];
        if ($search <> ''){
            $sql_search.= " WHERE product_type_name LIKE '%".$search."%'";
        }
        ...
        echo "<p align='right'><a href='insert_product_type.php' class='btn btn-success' role='button'>เพิ่มข้อมูล</a></p>";
        ...
        if ($result->num_rows > 0) { 
            ...
            while($result_array = $result->fetch_assoc()) {
                echo "<tr>";
                    echo "<td><center>".$result_array['product_type_id']."</center></td>";
                    echo "<td><center>".$result_array['product_type_name']."</center></td>";
                    echo "<td><center><a href='update_product_type.php?val=".$result_array['product_type_id']."'target='_self' class='btn btn-warning' role='button'>แก้ไข</a></center></td>";
                    echo "<td><center><a href='exec.php?val=".md5($result_array['product_type_id'])."&chk=delete_product_type'target='_self'onclick='return confirm(\"ยืนยันการลบข้อมูล\")' class='btn btn-danger' role='button'>ลบ</a></center></td>";
                echo "</tr>";
            }
            echo "</table>";  
        }else{
